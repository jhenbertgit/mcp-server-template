...
async function main() {
    const server = new Server({ name: "local-mcp-server", version: "0.1.0" }, {
        capabilities: {
            tools: {},
            resources: {},
            prompts: {},
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
        return {
            tools: [
                {
                    name: "get_weather",
                    description: "Get weather for a city. Provide 'city', optional 'units' (metric|imperial), and optional 'mode' ('current' | 'hourly' | 'daily'). For daily, you can also set 'days' (7-10).",
                    inputSchema: {
                        type: "object",
                        properties: {
                            ...
                            mode: {
                                type: "string",
                                description: "Data mode: 'current' (default), 'hourly' (next ~24h), or 'daily' (next 7-10 days)",
                                enum: ["current", "hourly", "daily"],
                            },
                            days: {
                                type: "number",
                                description: "For daily mode only: number of forecast days (7-10). Defaults to 7.",
                                minimum: 1,
                                maximum: 16,
                            },
                            format: {
                                type: "string",
                                description: "Response format: 'json' (default) or 'text' (compat mode returning stringified JSON)",
                                enum: ["json", "text"],
                            },
                        },
                        required: ["city"],
    ...
    server.setRequestHandler(CallToolRequestSchema, async (req) => {
        ...
        const defaultFormat = process.env.MCP_TEXT_OUTPUT === "1" ? "text" : "json";
        // Helper to emit either JSON or TEXT content
        const wrap = (fmt, payload) => fmt === "text"
        ...
        if (tool === "get_weather") {
            ...
            try {
                ...
                if (mode === "current") {
                    ...
                    const t = typeof temperature === "number" && Number.isFinite(temperature)
                        ? Number(temperature.toFixed(1))
                        : null;
                    const w = typeof windSpeed === "number" && Number.isFinite(windSpeed)
                ...
                if (mode === "hourly") {
                    ...
                    const times = Array.from({ length: count }, (_, i) => new Date((start + i * interval + response.utcOffsetSeconds()) * 1000));
                    ...
                    const items = Array.from({ length: max }, (_, i) => ({
                        ...
                        temperature: typeof temp[i] === "number"
                            ? Number(Number(temp[i]).toFixed(1))
                            : null,
                        temperature_unit: tempUnit,
                        wind_speed: typeof wind[i] === "number"
                ...
                const dTimes = Array.from({ length: dCount }, (_, i) => new Date((dStart + i * dInterval + response.utcOffsetSeconds()) * 1000));
                ...
                const dMax = Math.min(tMax.length, tMin.length, precip.length, windMax.length, dTimes.length);
                const daysOut = Array.from({ length: dMax }, (_, i) => ({
                    date: dTimes[i].toISOString().slice(0, 10),
                    t_max: typeof tMax[i] === "number"
                        ? Number(Number(tMax[i]).toFixed(1))
                        : null,
                    t_min: typeof tMin[i] === "number"
                        ? Number(Number(tMin[i]).toFixed(1))
                        : null,
                    temperature_unit: tempUnit,
                    precipitation_sum: typeof precip[i] === "number"
                        ? Number(Number(precip[i]).toFixed(1))
                        : null,
                    precipitation_unit: "mm",
                    wind_speed_10m_max: typeof windMax[i] === "number"
